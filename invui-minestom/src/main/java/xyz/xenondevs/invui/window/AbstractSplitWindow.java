...
public abstract class AbstractSplitWindow extends AbstractDoubleWindow {
    ...
    public AbstractSplitWindow(Player player, Component title, AbstractGui upperGui, AbstractGui lowerGui, Inventory upperInventory, boolean closeable, List<Consumer<String>> renameHandlers) {
        super(player, title, upperGui.getSize() + lowerGui.getSize(), upperInventory, closeable, renameHandlers);
        this.upperGui = upperGui;
        this.lowerGui = lowerGui;
    }
    ...
    @Override
    protected Pair<AbstractGui, Integer> getWhereClicked(InventoryPreClickEvent event) {
        AbstractInventory clicked = event.getInventory();
