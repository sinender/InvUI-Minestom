 ...
 * <ul>
 *     <li>
 *         There are no {@link ItemStack ItemStacks} of type {@link Material#AIR} or {@link ItemStack#amount()} ()} == 0.
 *         <br>
...
public abstract class Inventory {
     ...
     * {@link DataComponents} of an {@link ItemStack} in this inventory has changed.
     */
    ...
    public ItemPreUpdateEvent callPreUpdateEvent(@Nullable UpdateReason updateReason, int slot, @Nullable ItemStack previousItemStack, @Nullable ItemStack newItemStack) {
        ...
        if (preUpdateHandler != null) {
            try {
                ...
                InvUI.getInstance().getLogger().error("An exception occurred while handling an inventory event", t);
            }
        }
        return event;
    }
    ...
    public boolean isFull() {
        ...
        for (int slot = 0; slot < items.length; slot++) {
            ...
            if (item == null || item.amount() < getMaxStackSize(slot))
                return false;
        }
    ...
    public boolean contains(Predicate<ItemStack> predicate) {
        for (ItemStack item : getUnsafeItems()) {
            if (item != null && predicate.test(item.builder().build()))
                return true;
        }
    ...
    /**
     * Gets the {@link ItemStack#amount()} () ItemStack amount} of the {@link ItemStack} on the given slot.
     *
     * @param slot The slot
    ...
    public int getItemAmount(int slot) {
        ...
        return currentStack != null ? currentStack.amount() : 0;
    }
    ...
    public boolean forceSetItem(@Nullable UpdateReason updateReason, int slot, @Nullable ItemStack itemStack) {
        if (!shouldCallEvents(updateReason)) {
            ...
            ItemPreUpdateEvent event = callPreUpdateEvent(updateReason, slot, previousStack, itemStack != null ? itemStack.builder().build() : null);
    ...
    public int putItem(@Nullable UpdateReason updateReason, int slot, @NotNull ItemStack itemStack) {
        ...
        if (currentStack == null || currentStack.isSimilar(itemStack)) {
            ...
            if (currentAmount < maxStackSize) {
                int additionalAmount = itemStack.amount();
                int newAmount = Math.min(currentAmount + additionalAmount, maxStackSize);
                
                ItemStack newItemStack = itemStack.builder().build().withAmount(newAmount);
                
                if (shouldCallEvents(updateReason)) {
                    ItemStack currentStackC = currentStack != null ? currentStack.builder().build() : null;
                    ...
                    if (!event.isCancelled()) {
                        ...
                        int newAmountEvent = newItemStack != null ? newItemStack.amount() : 0;
                        int remaining = itemStack.amount() - (newAmountEvent - currentAmount);
    ...
    public int setItemAmount(@Nullable UpdateReason updateReason, int slot, int amount) {
        ...
        if (amount > 0) {
            newItemStack = currentStack.builder().build().withAmount(Math.min(amount, maxStackSize));
        } else {
            newItemStack = null;
        }
        ...
        if (shouldCallEvents(updateReason)) {
            ...
            if (!event.isCancelled()) {
                ...
                int actualAmount = newItemStack != null ? newItemStack.amount() : 0;
    ...
    private int addToPartialSlots(
        ...
        for (int slot = 0; slot < items.length; slot++) {
            ...
            if (currentStack == null)
                continue;
            int maxStackSize = getMaxSlotStackSize(slot, itemStack);
            if (currentStack.amount() >= maxStackSize)
                continue;
            if (!itemStack.isSimilar(currentStack))
                continue;
            
            // partial stack found, put items
            ItemStack newStack = itemStack.builder().amount(Math.min(currentStack.amount() + amountLeft, maxStackSize)).build();
            if (shouldCallEvents(updateReason)) {
                ItemPreUpdateEvent event = callPreUpdateEvent(updateReason, slot, currentStack.builder().build(), newStack);
                if (!event.isCancelled()) {
                    newStack = event.getNewItem();
                    setCloneBackingItem(slot, newStack);
                    callPostUpdateEvent(updateReason, slot, currentStack.builder().build(), newStack);
                    
                    int newStackAmount = newStack != null ? newStack.amount() : 0;
                    amountLeft -= newStackAmount - currentStack.amount();
                }
            } else {
                setDirectBackingItem(slot, newStack);
                amountLeft -= newStack.amount() - currentStack.amount();
            }
        }
    ...
    private int addToEmptySlots(
        ...
        for (int slot = 0; slot < items.length; slot++) {
            ...
            ItemStack newStack = itemStack.builder().amount(Math.min(amountLeft, getMaxSlotStackSize(slot, itemStack))).build();
            if (shouldCallEvents(updateReason)) {
    ...
    public int simulateSingleAdd(@NotNull ItemStack itemStack) {
        ...
        for (int slot = 0; slot < items.length; slot++) {
            ...
            amountLeft = Math.max(0, amountLeft - (maxStackSize - currentStack.amount()));
        }
    ...
    public int collectSimilar(@Nullable UpdateReason updateReason, @NotNull ItemStack itemStack) {
        return collectSimilar(updateReason, itemStack, itemStack.amount());
    }
    ...
    public int collectSimilar(@Nullable UpdateReason updateReason, @NotNull ItemStack template, int baseAmount) {
        ...
        if (amount < maxStackSize) {
            ...
            for (int slot = 0; slot < items.length; slot++) {
                ItemStack currentStack = items[slot];
                if (currentStack == null || currentStack.amount() >= maxStackSize || !template.isSimilar(currentStack))
                    continue;
    ...
    public int removeIf(@Nullable UpdateReason updateReason, @NotNull Predicate<@NotNull ItemStack> predicate) {
        ...
        for (int slot = 0; slot < items.length; slot++) {
            ItemStack item = items[slot];
            if (item != null && predicate.test(item.builder().build()) && setItem(updateReason, slot, null)) {
                removed += item.amount();
            }
        }
    ...
    private int takeFrom(@Nullable UpdateReason updateReason, int slot, int maxTake) {
        ...
        int amount = currentItemStack.amount();
        int take = Math.min(amount, maxTake);
        
        ItemStack newItemStack;
        if (take != amount) {
            newItemStack = currentItemStack.builder().amount(amount - take).build();
        } else newItemStack = null;
        
        if (shouldCallEvents(updateReason)) {
            ItemStack currentItemStackC = currentItemStack.builder().build();
            ...
            if (!event.isCancelled()) {
                ...
                int amountTaken = currentItemStack.amount() - (newItemStack == null ? 0 : newItemStack.amount());
