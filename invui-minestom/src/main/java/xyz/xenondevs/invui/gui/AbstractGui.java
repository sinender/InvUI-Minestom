...
public abstract class AbstractGui implements Gui, GuiParent {
     ...
     */
    public void handleClick(int slotNumber, Player player, Click clickType, InventoryPreClickEvent event) {
     ...
     * @param event   The {@link InventoryPreClickEvent} that was triggered
     */
    protected void handleInvSlotElementClick(SlotElement.InventorySlotElement element, InventoryPreClickEvent event) {
        ...
        if (inventory.isSynced(slot, clicked) || didClickBackgroundItem(player, element, inventory, slot, clicked)) {
            ...
            Class<? extends Click> aClass = event.getClick().getClass();
            if (aClass.equals(Click.Left.class)) {
                handleInvLeftClick(event, inventory, slot, player, technicallyClicked, cursor);
            } else if (aClass.equals(Click.Right.class)) {
                handleInvRightClick(event, inventory, slot, player, technicallyClicked, cursor);
            } else if (aClass.equals(Click.LeftShift.class) || aClass.equals(Click.RightShift.class)) {
                handleInvItemShift(event, inventory, slot, player, technicallyClicked);
            } else if (aClass.equals(Click.HotbarSwap.class)) {
                handleInvNumberKey(event, inventory, slot, player, technicallyClicked);
            } else if (aClass.equals(Click.OffhandSwap.class)) {
                handleInvOffHandKey(event, inventory, slot, player, technicallyClicked);
            } else if (aClass.equals(Click.DropCursor.class)) {
                handleInvDrop(false, event, inventory, slot, player, technicallyClicked);
            } else if (aClass.equals(Click.Double.class)) {
                handleInvDoubleClick(event, player, cursor);
            } else if (aClass.equals(Click.Middle.class)) {
                handleInvMiddleClick(event, inventory, slot, player);
            } else {
                InvUI.getInstance().getLogger().warn("Unknown click type: " + event.getClick().getClass().getSimpleName());
            }
        }
    }
    ...
    private boolean didClickBackgroundItem(Player player, SlotElement.InventorySlotElement element, Inventory inventory, int slot, ItemStack clicked) {
        String lang = player.getLocale().getLanguage();
    ...
    protected void handleInvLeftClick(InventoryPreClickEvent event, Inventory inventory, int slot, Player player, ItemStack clicked, ItemStack cursor) {
        ...
        if (cursor == null) {
            ...
            if (inventory.setItem(updateReason, slot, null))
                player.getInventory().setCursorItem(clicked);
            ...
            if (remains == 0) {
                ...
                player.getInventory().setCursorItem(cursor.builder().amount(remains).build());
            }
     ...
     */
    @SuppressWarnings("deprecation")
    protected void handleInvRightClick(InventoryPreClickEvent event, Inventory inventory, int slot, Player player, ItemStack clicked, ItemStack cursor) {
        // nothing happens if both cursor and clicked stack are empty
        if (clicked == null && cursor == null) return;
        ...
        if (cursor == null) {
            ...
            int clickedAmount = clicked.amount();
            int newClickedAmount = clickedAmount / 2;
            int newCursorAmount = clickedAmount - newClickedAmount;

            cursor = clicked.builder().build();

            clicked = clicked.builder()
                    .amount(newClickedAmount)
                    .build();
            cursor = cursor.builder()
                    .amount(newCursorAmount)
                    .build();
            ...
            ItemStack toAdd = cursor.builder().amount(1).build();
     ...
     * @param clicked   The {@link ItemStack} that was clicked
     */
    protected void handleInvItemShift(InventoryPreClickEvent event, Inventory inventory, int slot, Player player, ItemStack clicked) {
        if (clicked == null) return;

        ItemStack previousStack = clicked.builder().build();
        ...
        if (!updateEvent.isCancelled()) {
            ...
            clicked = clicked.withAmount(leftOverAmount);
    ...
    protected void handleInvNumberKey(InventoryPreClickEvent event, Inventory inventory, int slot, Player player, ItemStack clicked) {
        Window window = WindowManager.getInstance().getOpenWindow(player);
        if (window instanceof AbstractSingleWindow) {
            Click.HotbarSwap eventClick = (Click.HotbarSwap) event.getClick();
            PlayerInventory playerInventory = player.getInventory();
            int hotbarButton = eventClick.hotbarSlot();
            ItemStack hotbarItem = ItemUtils.takeUnlessEmpty(playerInventory.getItemStack(hotbarButton));

            UpdateReason updateReason = new PlayerUpdateReason(player, event);

            if (inventory.setItem(updateReason, slot, hotbarItem))
                playerInventory.setItemStack(hotbarButton, clicked);
        }
    }
     ...
     */
    // TODO: add support for merged windows
    protected void handleInvOffHandKey(InventoryPreClickEvent event, Inventory inventory, int slot, Player player, ItemStack clicked) {
        Window window = WindowManager.getInstance().getOpenWindow(player);
        if (window instanceof AbstractSingleWindow) {
            PlayerInventory playerInventory = player.getInventory();
            ItemStack offhandItem = ItemUtils.takeUnlessEmpty(player.getItemInOffHand());

            UpdateReason updateReason = new PlayerUpdateReason(player, event);

            if (inventory.setItem(updateReason, slot, offhandItem))
                player.setItemInOffHand(clicked);
        }
    }
     ...
     * @param clicked   The {@link ItemStack} that was clicked
     */
    protected void handleInvDrop(boolean ctrl, InventoryPreClickEvent event, Inventory inventory, int slot, Player player, ItemStack clicked) {
        if (clicked == null) return;
        ...
        if (ctrl) {
            ...
            InventoryUtils.dropItemLikePlayer(player, clicked.withAmount(1));
        }
     ...
     * @param cursor The {@link ItemStack} that is on the cursor
     */
    protected void handleInvDoubleClick(InventoryPreClickEvent event, Player player, ItemStack cursor) {
        if (cursor == null)
            return;
     ...
     */
    @SuppressWarnings("deprecation")
    protected void handleInvMiddleClick(InventoryPreClickEvent event, Inventory inventory, int slot, Player player) {
        ...
        if (cursor == null || cursor.isAir())
            return;
        player.getInventory().setCursorItem(cursor.withAmount(cursor.maxStackSize()));
    }
    ...
    public void handleItemShift(InventoryPreClickEvent event) {
        ...
        if (amountLeft != clicked.amount()) {
            if (amountLeft != 0) event.getInventory().setItemStack(event.getSlot(), clicked.withAmount(amountLeft));
            else event.getInventory().setItemStack(event.getSlot(), ItemStack.AIR);
        }
    }
